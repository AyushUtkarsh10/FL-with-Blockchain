...
# Check if the 'clients' directory exists, and create it if not
...
class Client:
    ...
    def update_model(self, model, steps):
        ...
        reset()
        t = time.time()
        worker = NNWorker(self.dataset['train_text'],
                          self.dataset['train_labels'],
                          self.dataset['test_text'],
                          self.dataset['test_labels'],
                          len(self.dataset['train_text']),
                          self.id,
                          steps)
        worker.build(model)
        worker.train()
    ...
    def send_update(self, update, cmp_time, baseindex):
        ...
        requests.post('http://{node}/transactions/new'.format(node=self.miner),
                      json={
                          ...
                          'datasize': len(self.dataset['train_text']),
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', default='data/sa.d', help='Path to dataset')
